...
class Cust_Win:
    def __init__(self, root):
        ...
        combo_idProofType = ttk.Combobox(labelframeleft, textvariable=self.var_id_proof, font=("arial", 12, "bold"),
                                         width=27, state="readonly")
        combo_idProofType["value"] = ("Huduma Number", "Driving License", "Passport")
        combo_idProofType.current(2)
        ...
        # address
        lblEmail = Label(labelframeleft, text="Address:", font=("arial", 12, "bold"), padx=2, pady=6)
        lblEmail.grid(row=10, column=0, sticky=W)

        txtEmail = ttk.Entry(labelframeleft, textvariable=self.var_address, width=29, font=("arial", 13, "bold"))
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(host="localhost", user="root", password="pierre", database="hotelmanagement")
        my_cursor = conn.cursor()
